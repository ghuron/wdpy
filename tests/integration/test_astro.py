...
class TestGetCanonicalName(TestCase):
    @mock.patch('wdpy.connectors.AstroItem.exec', return_value = {'HD 42': {}})
    def test_returns_main_id(self, mock:Optional[mock.MagicMock] = None):
        self.assertEqual('HD 42', AstroItem.get_canonical_name('HD 42'))
        if mock: mock.assert_called_once_with(
            "SELECT main_id FROM basic JOIN ident ON oid=oidref WHERE id = 'HD 42'"
        )

    @mock.patch('wdpy.connectors.AstroItem.exec', return_value = None)
    def test_returns_none_when_missing(self, *_):
        self.assertIsNone(AstroItem.get_canonical_name('HD 0'))

    @mock.patch('wdpy.connectors.AstroItem.exec', return_value = {'SN 1572A': {}})
    def test_escapes_single_quotes(self, mock:Optional[mock.MagicMock] = None):
        self.assertEqual('SN 1572A', AstroItem.get_canonical_name("Tycho's SN"))
        if mock: mock.assert_called_once_with(
            "SELECT main_id FROM basic JOIN ident ON oid=oidref WHERE id = 'Tycho''s SN'"
        )

class TestParseUrl(TestCase):
    @mock.patch('wdpy.connectors.astro.AstroItem.lookup', return_value='Q55882019')
    def test_parse_ads_encoded(self, api_search:Optional[mock.MagicMock]=None):
        value = AstroItem.parse_url('https://ui.adsabs.harvard.edu/abs/2018A%26A...609A.117T/abstract')
        self.assertEqual('Q55882019', value)
        if api_search: api_search.assert_called_with('P819', '2018A&A...609A.117T')
